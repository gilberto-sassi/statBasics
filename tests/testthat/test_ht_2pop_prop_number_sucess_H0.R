...
n1 <- 150
n2 <- 400
x1 <- rbinom(n1, 1, p1_pop)
x2 <- rbinom(n2, 1, p2_pop)
delta <- p1_pop - p2_pop
conf_level <- 0.99
sig_level <- 0.01
...
prop_test <- function(...) {
  ...
  ci <- ci_2pop_bern(x1, x2, n1, n2, conf_level = values$conf_level,
                    type = type)
...
output1 <- ht_2pop_prop(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "two.sided", conf_level = conf_level, sig_level = sig_level)
output2 <- prop_test(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "two.sided", conf_level = conf_level, correct = F, sig_level = sig_level)
...
output1 <- ht_2pop_prop(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "greater", conf_level = conf_level, sig_level = sig_level)
output2 <- prop_test(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "greater", conf_level = conf_level, correct = FALSE, sig_level = 0.01)
...
output1 <- ht_2pop_prop(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "less", conf_level = conf_level, sig_level = sig_level)
output2 <- prop_test(sum(x1), sum(x2), n1, n2, delta = delta, alternative = "less", conf_level = conf_level, correct = FALSE, sig_level = 0.01)
