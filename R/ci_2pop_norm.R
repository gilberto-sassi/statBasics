#' Confidence Interval for Normal Distribution - 2 populations
#'
#' Computes the confidence interval for the difference of 2 population means or
#' computes the confidence interval for the ratio of 2 population variances
#' according the \code{parameter} argument.
#'
...
ci_2pop_norm <- function(x, y, sd_pop_1 = NULL, sd_pop_2 = NULL, var_equal = FALSE, parameter = "mean", conf_level= 0.95, type = "two.sided", na.rm = F) {
  ...
  if ((!is.null(sd_pop_1) | !is.null(sd_pop_2)) & parameter %in% "variance") {
    stop("You should not build a confidence interval for variance ratio if you know  populational variances.")
  }
...
# unknown variances and differents ----------------------------------------
